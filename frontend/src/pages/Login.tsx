...
const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    // Client-side required-field checks run before calling the API
    ...
    try {
      // totpCode is optional and only required when the account has MFA enabled
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-[var(--bg-page)] px-3 py-6 sm:px-6 lg:px-8">
      <div className="w-full max-w-5xl overflow-hidden rounded-3xl bg-white shadow-2xl md:grid md:grid-cols-[1.05fr_0.95fr]">
        <div
          className="hidden flex-col justify-between p-8 text-white relative md:flex"
          style={{ background: "var(--hero-gradient)" }}
        >
          <div
            className="absolute top-0 right-8 h-16 w-10"
            style={{ backgroundColor: "var(--color-accent)" }}
          />
          <div>
            <Music4
              className="mb-4 h-12 w-12 text-rose-100"
              aria-hidden="true"
            />
            <h1 className="text-4xl font-bold font-display">Perform Pro</h1>
            <p className="mt-3 text-sm text-cyan-100">
              Intelligent musician task management with AI-driven performance
              evaluation.
            </p>
          </div>
          <p className="text-xs text-cyan-100/90">
            Secure role-based access for musicians and administrators.
          </p>
        </div>
        ...
        <div className="p-4 sm:p-6 md:p-8">
          ...
          <div className="mb-6 text-center">
            ...
            <h1
              className="text-2xl font-bold font-display sm:text-3xl"
          ...
          <form className="space-y-5" onSubmit={handleSubmit}>
            ...
            {error && (
              <div className="text-red-600 text-sm text-center bg-red-50 rounded-lg px-4 py-2">
                {error}
              </div>
            )}
            ...
            <button
              ...
              style={{
                backgroundColor: isLoading ? "#991b1b" : "var(--color-accent)",
              }}
            >
