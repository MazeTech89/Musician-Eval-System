...
def upgrade():
    ...
    # Add S3 key column
    op.add_column(
        "performances", sa.Column("audio_s3_key", sa.String(500), nullable=True, unique=True)
    )

    # Add file metadata columns
    op.add_column("performances", sa.Column("file_size_bytes", sa.Integer(), nullable=True))

    # Add upload timestamp
    op.add_column("performances", sa.Column("uploaded_at", sa.DateTime(), nullable=True))
